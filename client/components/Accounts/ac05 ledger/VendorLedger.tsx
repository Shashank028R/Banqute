...
import { Download, Printer, Filter, Search, Store, CreditCard, IndianRupee } from 'lucide-react';
import { formatCurrency } from '../../../lib/utils';
import { vendorLedgerTransactions, vendorLedgerSummary } from './mockData';
    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-500 font-medium">Total Amount Payable</p>
            <h3 className="text-2xl font-bold text-gray-900">{formatCurrency(vendorLedgerSummary.totalAmountPayable)}</h3>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-bold text-emerald-600">{formatCurrency(vendorLedgerSummary.totalPaid)}</h3>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-500 font-medium">Outstanding Amount</p>
            <h3 className="text-2xl font-bold text-red-600">{formatCurrency(vendorLedgerSummary.outstandingAmount)}</h3>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
        <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <h2 className="text-xl font-bold text-gray-800">Vendor Ledger</h2>
          <div className="flex flex-wrap gap-3 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-64">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
              <input 
                type="text" 
                placeholder="Search vendor, transaction..." 
            ...
            <button className="flex items-center gap-2 px-4 py-2 border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
              <Store size={20} />
              <span className="hidden sm:inline">View Vendor</span>
            </button>
            <button className="flex items-center gap-2 px-4 py-2 border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
              <Printer size={20} />
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4 text-right">Debit (Payment)</th>
                <th className="px-6 py-4 text-right">Credit (Adjustment)</th>
                <th className="px-6 py-4 text-right">Balance</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 text-sm">
              {vendorLedgerTransactions.map(tx => (
